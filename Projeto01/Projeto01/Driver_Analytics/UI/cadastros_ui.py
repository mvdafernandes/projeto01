...
def _set_receita_fields(row: pd.Series | None) -> None:
    ...
    st.session_state["cad_receita_km_total_rodado"] = float(row.get("km_rodado_total", 0.0)) if row is not None else 0.0
...
def pagina_cadastros() -> None:
    ...
    with tab_receitas:
        ...
        with st.form("cad_receita_form"):
            ...
            km_total_rodado = st.number_input("KM total rodado do veículo", min_value=0.0, key="cad_receita_km_total_rodado")
                ...
                if salvar:
                    if data_valida is None:
                        ...
                        service.criar_receita(
                            ...
                            int(tempo_total),
                            observacao,
                            km_rodado_total=float(km_total_rodado),
                        )
                        st.success("Receita salva com sucesso.")
                        _reset_fields([
                            "cad_receita_selected_id", "cad_receita_last_selected_id", "cad_receita_data",
                            "cad_receita_valor", "cad_receita_km", "cad_receita_km_total_rodado", "cad_receita_tempo", "cad_receita_obs",
