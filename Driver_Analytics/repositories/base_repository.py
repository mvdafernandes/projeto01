...
class BaseRepository:
    ...
    def _list_remote_rows(self, order_by: str | None = None) -> list[dict]:
        """Read rows from Supabase strictly scoped to the authenticated user."""
        ...
        query = client.table(self.table_name).select("*").eq("user_id", int(user_id))
        if order_by:
    ...
    def _require_user_id(self) -> int:
        user_id = self._current_user_id()
        if user_id is None:
            raise RuntimeError("Usuario autenticado e obrigatorio para operar no repositorio remoto.")
        return int(user_id)

    def _with_user_id(self, payload: dict) -> dict:
        out = dict(payload)
        out["user_id"] = self._require_user_id()
        return out
