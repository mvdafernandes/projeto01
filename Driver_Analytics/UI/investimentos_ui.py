...
def _aportes_por_categoria(df: pd.DataFrame) -> pd.DataFrame:
    ...
    aportes = df[df["aporte"] > 0].groupby("categoria", as_index=False)["aporte"].sum()
    if aportes.empty:
...
def _rendimentos_por_categoria(df: pd.DataFrame) -> pd.DataFrame:
    """Aggregate rendimento totals by category from rendimento snapshots (aporte == 0)."""

    if df.empty:
        return pd.DataFrame({"categoria": CATEGORIAS, "rendimento": [0.0, 0.0], "participacao_pct": [0.0, 0.0]})
    rend = df[df["aporte"] <= 0].groupby("categoria", as_index=False)["rendimento"].sum()
    if rend.empty:
...
def pagina_investimentos() -> None:
    ...
    titulo_secao("Resumo")
    patrimonio = float(df_periodo.sort_values(by=["data", "id"]).iloc[-1]["patrimonio total"])
    total_aporte = float(df_periodo["aporte"].sum())
    lucro_periodo = float(df_periodo[df_periodo["aporte"] <= 0]["rendimento"].sum())
    patrimonio_base = _patrimonio_base_periodo(df, data_inicial)
    rent_periodo = (lucro_periodo / patrimonio_base) * 100.0 if patrimonio_base > 0 else 0.0
    ...
    st.caption("Aportes só incrementam patrimônio. Lucro e rentabilidade consideram apenas registros de rendimento (aporte = 0).")
