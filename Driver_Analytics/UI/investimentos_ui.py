...
def _rendimentos_por_categoria(df: pd.DataFrame) -> pd.DataFrame:
    ...
    rend = df[df["rendimento"] != 0].groupby("categoria", as_index=False)["rendimento"].sum()
    if rend.empty:
...
def pagina_investimentos() -> None:
    ...
    aportes_brutos = float(df_periodo[df_periodo["aporte"] > 0]["aporte"].sum())
    retiradas_total = float(abs(df_periodo[df_periodo["aporte"] < 0]["aporte"].sum()))
    aportes_liquidos = float(df_periodo["aporte"].sum())
    lucro_periodo = float(df_periodo["rendimento"].sum())
