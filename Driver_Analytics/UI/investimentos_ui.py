...
def _rendimentos_por_categoria(df: pd.DataFrame) -> pd.DataFrame:
    """Aggregate rendimento totals by RF/RV category with complete category set."""

    if df.empty:
        return pd.DataFrame({"categoria": CATEGORIAS, "rendimento": [0.0, 0.0]})

    grouped = (
        df.groupby("categoria", as_index=False)["rendimento"]
        .sum()
        .rename(columns={"rendimento": "rendimento"})
    )
