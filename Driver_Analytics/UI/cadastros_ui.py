...
def pagina_cadastros() -> None:
    ...
    with tab_despesas:
        ...
        categoria_em_edicao = str(st.session_state.get("cad_despesa_categoria", "")).strip()
        if categoria_em_edicao and categoria_em_edicao not in categorias_existentes:
        ...
        with st.form("cad_despesa_form"):
            ...
            categoria_existente = st.selectbox(
                "Categoria existente",
                options=[""] + categorias_existentes,
                index=0,
                key="cad_despesa_categoria",
            )
            nova_categoria = st.text_input("Nova categoria (opcional)", key="cad_despesa_nova_categoria")
            ...
            data_valida = _safe_date_or_none(data)
            categoria_escolhida = str(nova_categoria).strip() or str(categoria_existente).strip()
            ...
            try:
                if salvar:
                    if data_valida is None:
                        ...
                        st.warning("Informe ou selecione uma categoria.")
                    else:
                        ...
                        _reset_fields([
                            ...
                            "cad_despesa_categoria", "cad_despesa_nova_categoria", "cad_despesa_valor",
