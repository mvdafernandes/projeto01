...
def _set_invest_rendimento_fields(row: pd.Series | None) -> None:
    ...
    st.session_state["cad_inv_rend_aporte_base"] = float(row["aporte"]) if row is not None else 0.0
...
def pagina_cadastros() -> None:
    ...
    with tab_investimentos:
        ...
        with sub_aportes:
            st.caption("Este formulário controla lançamentos de aporte e patrimônio. Rendimentos são tratados no formulário ao lado.")
            with st.form("cad_investimento_form"):
                ...
                data_valida = _safe_date_or_none(data)
                rendimento_atual = float(st.session_state.get("cad_invest_rendimento", 0.0))
                ...
                try:
                    if salvar:
                        if data_valida is None:
                            ...
                            service.criar_investimento(data_valida.isoformat(), categoria, float(aporte), 0.0, 0.0, float(patrimonio_total))
        ...
        with sub_rendimentos:
            ...
            with st.form("cad_invest_rendimento_form"):
                ...
                aporte_base = float(st.session_state.get("cad_inv_rend_aporte_base", 0.0))
                    ...
                    if salvar_r:
                        if data_valida_r is None:
                            ...
                            _reset_fields([
                                ...
                                "cad_inv_rend_rendimento", "cad_inv_rend_patrimonio", "cad_inv_rend_aporte_base",
