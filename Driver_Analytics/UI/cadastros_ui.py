...
def pagina_cadastros() -> None:
    ...
    with tab_despesas:
        ...
        with st.form("cad_despesa_form"):
            ...
            if str(tipo_despesa_label).strip().lower() == "recorrente":
                recorrencia_label = st.selectbox(
                    "Recorrência",
                    options=RECORRENCIA_DESPESA_OPTIONS,
                    key="cad_despesa_recorrencia_tipo",
                )
                if recorrencia_label == RECORRENCIA_DESPESA_LABELS["PERSONALIZADO"]:
                    recorrencia_meses = int(
                        st.number_input(
                            ...
                            min_value=1,
                            step=1,
                            key="cad_despesa_recorrencia_meses",
                        )
                    )
            subcategoria_fixa = ""
            ...
            recorrencia_tipo = {
                "Indeterminado": "INDETERMINADO",
                "Personalizado (N meses)": "PERSONALIZADO",
            }.get(str(recorrencia_label), "INDETERMINADO")
                ...
                if salvar:
                    if data_valida is None:
                        ...
                        _reset_fields([
                            ...
                            "cad_despesa_recorrencia_tipo", "cad_despesa_recorrencia_meses",
    ...
    with tab_controle:
        ...
        st.caption(
            ...
            "O formato é CSV versionado para facilitar recuperação."
        )
