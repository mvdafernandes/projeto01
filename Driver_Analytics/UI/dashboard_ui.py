...
def _recent_rows(df, limit=10):
    data_col = _resolve_data_column(df)
    if data_col is None:
        return df.head(limit).copy(), None

    try:
        return df.sort_values(data_col, ascending=False).head(limit).copy(), data_col
    except (KeyError, TypeError, ValueError):
        return df.head(limit).copy(), data_col
...
def pagina_dashboard():
    ...
    data_col_receitas = _resolve_data_column(df_receitas)
    data_col_despesas = _resolve_data_column(df_despesas)

    if not df_receitas.empty and data_col_receitas is not None:
        df_receitas[data_col_receitas] = pd.to_datetime(df_receitas[data_col_receitas], errors="coerce")
    if not df_despesas.empty and data_col_despesas is not None:
        df_despesas[data_col_despesas] = pd.to_datetime(df_despesas[data_col_despesas], errors="coerce")
    ...
    with col1:
        ...
        df_r, data_col_r = _recent_rows(df_receitas, limit=10)
        if not df_r.empty and data_col_r is not None:
            df_r[data_col_r] = pd.to_datetime(df_r[data_col_r], errors="coerce").dt.date
        st.dataframe(df_r, width="stretch")
    with col2:
        titulo_secao("Despesas recentes")
        df_d, data_col_d = _recent_rows(df_despesas, limit=10)
        if not df_d.empty and data_col_d is not None:
            df_d[data_col_d] = pd.to_datetime(df_d[data_col_d], errors="coerce").dt.date
