...
def pagina_cadastros(conn):
    ...
    with st.form("cad_receita"):
        ...
        tempo = st.number_input("Tempo trabalhado (min)", min_value=0, step=1, key="cad_tempo_r")
        observacao = st.text_input("Observação", key="cad_obs_r")
        submit = st.form_submit_button("Salvar Receita")
        if submit:
            inserir_receita(data.isoformat(), valor, km, tempo, observacao)
